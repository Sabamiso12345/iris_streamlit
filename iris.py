...
#目標値の名前を0,1,2から花の名前に変更
df.loc[df['target']==0,'target']='setosa'
df.loc[df['target']==1,'target']='varsicolor'
df.loc[df['target']==2,'target']='virginica'

#予測モデルの構築（ロジスティック回帰）
x = iris.data[:,[0,2]]#すべての行の0列目と2列目（0列目：sepal length ２列目:petal length
y = iris.target
...
sepalValue = st.sidebar.slider('sepal length(cm)',min_value = 0.0, max_value = 10.0,step = 0.1)#step=0.1:0.1刻みで設定できる
petalValue = st.sidebar.slider('prtal lemgth(cm)',min_value = 0.0, max_value = 10.0,step = 0.1)

#メインパネル
st.title('Iris Classifier')
st.write('## Input Value')

# インプットデータ（1行のデータフレーム）#教材そのままでは無理。
...
value_df = pd.concat([value_df,record.to_frame().T],ignore_index=True)#pd.dataframeのappendは廃止になった
